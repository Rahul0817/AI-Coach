...
def engineer_features(raw: dict) -> dict[str, float]:
    ...
    activity_value = activity.value if hasattr(activity, "value") else str(activity)
    out["activity_score"] = float(ACTIVITY_SCORE_MAP.get(activity_value, 2))

    # --- androgenic symptom burden (0–4) ---
    out["androgenic_symptom_count"] = float(sum(int(raw[m]) for m in ANDROGENIC_MARKERS))
    ...
    out["cycle_deviation"] = float(abs(out["cycle_length_days"] - REFERENCE_CYCLE_LENGTH))
...
def engineer_frame(frame):  # type: ignore[no-untyped-def]
    ...
    **must** produce identical values — training/serving skew is the single
    ...
    out["androgenic_symptom_count"] = frame[ANDROGENIC_MARKERS].astype(float).sum(axis=1)
    ...
    out["cycle_deviation"] = (out["cycle_length_days"] - REFERENCE_CYCLE_LENGTH).abs()
